...
import { fetchDocuments, fetchEntities, fetchRelations, uploadDocument, mergeNodes, queryGraph } from "./api";
...
function Button({ onClick, children, variant = "primary", disabled = false }: { onClick?: () => void; children: React.ReactNode; variant?: "primary" | "secondary"; disabled?: boolean }) {
    return (
        <button className={`btn ${variant === "secondary" ? "secondary" : ""}`} onClick={onClick} disabled={disabled}>
            {children}
        </button>
    );
}
...
function SearchSection() {
    ...
    return (
        <section>
            ...
            <form onSubmit={handleSearch} style={{ display: "flex", gap: "1rem", marginBottom: "2rem", marginTop: "1rem" }}>
                <input
                    type="text"
                ...
                <Card className="graph-viz">
                    {data.nodes.length === 0 ? (
                        ...
                        <div style={{ padding: "1rem", width: "100%", height: "100%", overflow: "auto" }}>
                            <h4 style={{ marginBottom: "1rem" }}>Found {data.nodes.length} Nodes, {data.edges.length} Edges</h4>
                            <ul className="data-list">
                                {data.nodes.map(node => (
                                    <li key={node.id} className="list-item">
                                        ...
                                        {node.description && <span style={{ marginTop: "0.5rem" }}>{node.description}</span>}
                                    </li>
                                ))}
                            </ul>
                        </div>
...
function DataLists() {
    const [activeTab, setActiveTab] = useState<"documents" | "entities" | "relations">("documents");
    ...
    return (
        <Card>
            <div className="tabs">
                ...
                <button className={`tab ${activeTab === "entities" ? "active" : ""}`} onClick={() => setActiveTab("entities")}>Entities ({entities.length})</button>
                <button className={`tab ${activeTab === "relations" ? "active" : ""}`} onClick={() => setActiveTab("relations")}>Relations ({relations.length})</button>
            </div>
            ...
            <ul className="data-list">
                {activeTab === "documents" && documents.map(doc => (
                    <li key={doc.id} className="list-item">
                        <h4>{doc.title}</h4>
                        <span>{new Date(doc.createdAt).toLocaleDateString()} • {doc.securityLevel}</span>
                    </li>
                ))}
                {activeTab === "entities" && entities.map(ent => (
                ...
                {activeTab === "relations" && relations.map(rel => (
                    <li key={rel.id} className="list-item">
                        <h4>{rel.type}</h4>
                        <span>Source: {rel.sourceEntityId} → Target: {rel.targetEntityId}</span>
            ...
            <div className="modal" onClick={e => e.stopPropagation()}>
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div style={{ display: "flex", justifyContent: "flex-end", gap: "1rem" }}>
                        ...
                        <Button disabled={!file || uploading}>{uploading ? "Uploading..." : "Upload"}</Button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div className="app-container">
            <header style={{ marginBottom: "3rem", borderBottom: "1px solid var(--card-border)", paddingBottom: "1rem" }}>
                ...
                <p style={{ fontSize: "1.1rem", maxWidth: "600px" }}>
                    Your autonomous Personal Chief of Staff. Using local AI to map your professional relationships and discover hidden connections in your documents.
                </p>
            </header>
